...
import { BaseError, decodeErrorResult, decodeEventLog, erc20Abi, keccak256, zeroAddress, zeroHash, type Abi } from 'viem'
import { fromEnv, loadDeployment, readJson, RECORDS_DIR, type Ctx } from './config.ts'
import { compileGuarded, compileGuardedV2, encodeGuardReport, guardArtifact, guardTestForwarderArtifact, buildTestReportTx, type GuardCaps, type GuardReport } from './guard.ts'
import { concentrationBounds } from './concentrated.ts'
import { checkSepoliaAssets, SEPOLIA } from './sepolia.ts'
...
export async function runGuardDemo(ctx: Ctx, d: Deployment, recordsDir = RECORDS_DIR) {
  if (![31337, 84532, 11155111].includes(ctx.chain.id) || d.chainId !== ctx.chain.id || await ctx.pc.getChainId() !== ctx.chain.id) throw new Error('guard demo requires matching local, Base Sepolia or Ethereum Sepolia chain')
  const official = ctx.chain.id === SEPOLIA.chainId
  if (official) {
    await checkSepoliaAssets(ctx.pc)
    if (d.aqua.toLowerCase() !== SEPOLIA.aqua || d.tokens.WETH?.toLowerCase() !== SEPOLIA.WETH || d.tokens.USDC?.toLowerCase() !== SEPOLIA.USDC) throw new Error('Sepolia demo requires canonical Aqua and WETH / Circle USDC')
  }
  const tokens = official ? [d.tokens.WETH!, d.tokens.USDC!] : [d.tokens.mWETH!, d.tokens.mUSDC!]
  const receiver = official ? readJson('artifacts/AquaGuardV2.json') as { abi: Abi; bytecode: Hex } : guardArtifact
  ...
  const dep = async (artifact: typeof guardArtifact, args: unknown[]) => {
    ...
    rec.execution(`deploy:${artifact === guardTestForwarderArtifact ? 'GuardTestForwarder' : official ? 'AquaGuardV2' : 'AquaGuard'}`, receipt)
  ...
  const guard = await dep(receiver, [forwarder, d.router, zeroHash, zeroAddress, true])
  const caps: GuardCaps = official
    ? { maxAmount0PerSwap: 4n * 10n ** 14n, maxAmount1PerSwap: 1_000_000n, maxPostBalance0: 6n * 10n ** 15n, maxPostBalance1: 15_000_000n }
    : { maxAmount0PerSwap: 10n ** 18n, maxAmount1PerSwap: 1_000_000_000n, maxPostBalance0: 3n * 10n ** 18n, maxPostBalance1: 7_000_000_000n }
  const p = await fixedParams(ctx, d, readJson(official ? 'params/sepolia.json' : 'params/fixed.json'), { revision: 1, salt: BigInt(`0x${randomBytes(8).toString('hex')}`), feeBps: 0 })
  // Explicit recording fixture (2500 USDC/WETH reference), not a live market price.
  if (official) p.program = { ...p.program, kind: 'concentrated', ...concentrationBounds(tokens, [18, 6], '2000', '3000') }
  const s = (official ? compileGuardedV2 : compileGuarded)(p, guard, caps)
  const usdcIn = official ? 500_000n : 100_000_000n
  const wethIn = official ? 10n ** 14n : 10n ** 16n
  ...
  await publish(3)
  const swaps = [await takerSwap(ctx, d, s, s.tokens[1]!, usdcIn, 50, rec), await takerSwap(ctx, d, s, s.tokens[0]!, wethIn, 50, rec)]
  // Capture an allowed quote before disabling token1-in. The later tx must consult the new report.
  await quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, usdcIn)
  await publish(1)
  await send(ctx, ctx.taker, buildTx.approve(s.tokens[1]!, d.router, usdcIn), 'taker-approve-for-rejection', rec)
  const rejectedTx = buildTx.swap(d, s, s.tokens[1]!, s.tokens[0]!, usdcIn, takerTraits(0n))
  try {
    ...
    assert.equal(decodeErrorResult({ abi: receiver.abi, data: cause.data }).errorName, 'DirectionDisabled')
  }
  ...
  swaps.push(await takerSwap(ctx, d, s, s.tokens[0]!, wethIn, 50, rec))
  await publish(0)
  ...
  const summary = { mode, chainId: ctx.chain.id, aqua: d.aqua, router: d.router, forwarder, guard, guardVersion: official ? 2 : 1, tokens,
