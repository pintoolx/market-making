...
/// @notice Prototype of the report-v1 proposal. Only the fixed, zero-fee guarded template is supported.
/// @dev Immutable transport configuration. Simulation must use a separate testnet deployment.
contract AquaGuard is IReportReceiver, IExtruction, IStaticExtruction {
    ...
    function extruction(
        ...
        if (args.length != 105 || uint8(args[0]) != 1) revert InvalidEnvelope();
        ...
        uint256 postIn = swap.balanceIn + swap.amountIn;
        uint256 postOut = swap.balanceOut - swap.amountOut;
