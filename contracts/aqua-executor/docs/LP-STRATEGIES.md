...
The curve emits Concentrate opcode 18, then XYC opcode 17. Concentrate changes VM balance registers to **virtual pricing reserves**. Therefore `compileGuardedV2` requires the separate `AquaGuardV2` contract, whose terminal gate reads actual inventory from `router.AQUA().rawBalances`. It preserves all VM registers and only checks the proposed real post-swap balances. Version-2 envelopes fail on Guard v1; existing Guard v1 deployment artifacts are not upgraded. Guard V2 supports the flat-fee instruction: SwapVM restores gross input before the terminal gate, and the Guard uses that gross value for caps and actual Aqua inventory. Protocol and dynamic fees remain disabled.
...
Version 1 accepts XYC/Pegged with `feeBps: 0`; version 2 accepts all three curves and fixed LP input fees. `compileExecution` reconstructs the exact guarded bytecode from the persisted recipe; unknown fields, invalid versions and old specialized markers are rejected. Normal ship/rebalance/swap/dock/monitor requests and recovery use this compiler. **Every new strategyHash needs its own report**, including after changing range, salt, fee or caps. Atomic dock/ship succeeds without a report but trading remains disabled until the authorized transport delivers one. The controller never fabricates that authorization.
