...
test('standing reports survive time passage, preserve caps, and support durable Maker revocation', async () => {
  const { ctx, d, rec } = f
  const s = compileGuardedV2(params(), guard, caps)
  await f.activate(s, caps)
  await f.submit(s, { schemaVersion: 2, validUntil: 0 }, caps)
  const read = (name: string) => ctx.pc.readContract({ address: guard, abi: artifact.abi, functionName: name, args: [ctx.maker.account.address, s.strategyHash] })
  const acceptedBlock = await read('latestReportBlock')
  assert.ok(BigInt(acceptedBlock as bigint) > 0n)
  await ctx.pc.request({ method: 'evm_increaseTime' as never, params: [3600] as never })
  await ctx.pc.request({ method: 'evm_mine' as never })
  await takerSwap(ctx, d, s, s.tokens[1]!, 10_000_000n, 50, rec)
  await assert.rejects(quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, caps.maxAmount1PerSwap + 1n))
  assert.equal(await read('latestReportBlock'), acceptedBlock, 'swaps do not renew authorization')
  // Neither malformed standing reports nor expired legacy leases can override it.
  const now = Number((await ctx.pc.getBlock()).timestamp)
  await assert.rejects(f.submit(s, { schemaVersion: 2, nonce: 2n, validUntil: now + 600 }, caps))
  await assert.rejects(f.submit(s, { schemaVersion: 1, nonce: 2n, validAfter: now - 700, validUntil: now - 100 }, caps))
  const revoke = (revoked: boolean) => send(ctx, ctx.maker, { to: guard, data: encodeFunctionData({ abi: artifact.abi, functionName: 'setStrategyRevoked', args: [s.strategyHash, revoked] }) }, 'maker-revocation', rec)
  await revoke(true)
  await assert.rejects(quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, 10_000_000n), strategyNotActive)
  await assert.rejects(f.submit(s, { schemaVersion: 2, validUntil: 0, nonce: 2n }, caps))
  await revoke(false)
  await assert.rejects(quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, 10_000_000n), strategyNotActive)
  await f.submit(s, { schemaVersion: 2, validUntil: 0, nonce: 2n }, caps)
  await quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, 10_000_000n)
  await f.submit(s, { schemaVersion: 2, validUntil: 0, nonce: 3n, allowedDirections: 0 }, caps)
  await assert.rejects(quote(ctx, d, s, s.tokens[1]!, s.tokens[0]!, 10_000_000n), strategyNotActive)
  await send(ctx, ctx.maker, buildTx.dock(d, s), 'dock-standing', rec)
})
