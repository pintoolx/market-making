...
The PinTool frontend is the creator-facing canvas where on-chain quants wire, test, and deploy automated Web3 workflows without touching raw smart contracts. We built a highly opinionated experience on top of Next.js 14, Solana wallet adapters, and a bespoke node editor so our users can orchestrate complex strategies with the same ease as dragging blocks.
...
- **Seamless Auth** – Wallet-only onboarding powered by Supabase Web3 auth. The hover-driven account menu reroutes builders to their saved workflows or back to the canvas without ever blocking the main editor.
...
| Layer | Highlights |
| ----- | ---------- |
| **App Router** | Fully typed Next.js 14 app directory with React Server Components where it makes sense, client components for the interactive canvas. |
...
Navigate to `http://localhost:3000` and connect a Solana wallet (Devnet by default). To explore the creator flow:
