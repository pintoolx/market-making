...
with sync_playwright() as p:
    ...
    send('單項：只先保存 WETH 上限 0.05，其他還沒決定。')
    finished(2)
    expect(summary.get_by_role('row', name='WETH 0.05 尚未設定', exact=True)).to_be_visible()
    expect(summary.get_by_role('row', name='USDC 尚未設定 尚未設定', exact=True)).to_be_visible()
    expect(summary.get_by_text('單筆報價幣上限', exact=True)).to_be_visible()
    expect(summary.get_by_text('公開設定完整', exact=True)).to_have_count(0)
