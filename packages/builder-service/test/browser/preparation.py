...
with sync_playwright() as p:
    ...
    def connect_maker():
        ...
        expect(page.get_by_role('button', name='Check liquidity and test execution')).to_be_enabled()
    connect_maker()
    ...
    # A signed Provider instance can queue its first report without a fabricated
    # binding or asset signature. This fixture has no report broadcaster.
    dialog.get_by_role('button', name='Compile current draft').click()
    expect(dialog.get_by_text('Current v4 compilation saved')).to_be_visible()
    expect(dialog.get_by_role('region', name='Guard report binding')).to_have_count(0)
    dialog.get_by_role('button', name='Enable automatic event updates').click()
    expect(dialog.get_by_text('Event management enabled', exact=True)).to_be_visible()
    expect(dialog.get_by_text('First evaluation is queued. Report acceptance is pending.')).to_be_visible()
    expect(dialog.get_by_text('Accepted onchain', exact=True)).to_have_count(0)
    dialog.get_by_role('button', name='Stop automatic updates').click()
    expect(dialog.get_by_text('Event management is not authorized.')).to_be_visible()
    ...
    assert all('wants you to sign in' in m or m.startswith('Pintool Builder automation consent v1\n') or m.startswith('Pintool Builder revoke automation consent v1\n') for m in signed), 'Preparation unexpectedly requested a wallet signature'
    assert not errors, errors
    browser.close()
print('PASS: Maker inventory/evidence labels, real compilation, idempotent lost-response recovery, mock background simulation/cancel/reload, incomplete coverage, failure, stale edit/restore, session expiry and mobile; first authorization consent/stop without an existing report; no asset signatures/page errors')
