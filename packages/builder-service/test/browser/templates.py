...
with sync_playwright() as p:
    ...
    page.get_by_role('button', name='Private policy and publication').click()
    dialog = page.get_by_role('dialog', name='Publish Provider template')
    ...
    dialog.get_by_label('Rule 2 Allow Maker to buy WETH').check()
    dialog.get_by_label('Rule 2 WETH per-swap limit').fill('0.01')
    dialog.get_by_label('Rule 2 USDC per-swap limit').fill('100')
    dialog.get_by_role('button', name='+ Add rule').click()
    dialog.get_by_label('Rule 3 Allow Maker to sell WETH').check()
    ...
    expect(dialog.get_by_text('Template saved. Makers can select this version to create their own drafts. The private form has been cleared.')).to_be_visible()
    ...
    catalog = page.get_by_role('dialog', name='Choose Provider template')
    catalog.get_by_role('button', name=re.compile(re.escape(public_title) + '.*Version 1')).click()
    expect(catalog.get_by_text('Provider signature verified. Once selected, your pinned version will not update automatically.')).to_be_visible()
    ...
    catalog.get_by_role('button', name='Use template and continue designing').click()
    ...
    expect(page.get_by_text('Maker draft · Not registered')).to_be_visible()
    ...
    expect(catalog.get_by_role('button', name='Use template and continue designing')).to_have_count(0)
    ...
    expect(page.get_by_role('button', name='Private policy and publication')).to_be_enabled()
