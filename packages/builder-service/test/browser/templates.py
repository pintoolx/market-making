"""Local actual UI/API/PostgreSQL/encryption acceptance; fixture Privy/wallet/model only."""
import re
from uuid import uuid4
from pathlib import Path
from playwright.sync_api import sync_playwright, expect

artifacts = Path('.cache/builder')
artifacts.mkdir(parents=True, exist_ok=True)
with sync_playwright() as p:
    public_title = '模板驗收-' + uuid4().hex[:8]
    browser = p.chromium.launch(headless=True)
    page = browser.new_page(viewport={'width': 1440, 'height': 1100})
    errors, sent = [], []
    page.on('pageerror', lambda error: errors.append(str(error)))
    page.on('request', lambda req: sent.append({'url': req.url, 'body': req.post_data or '', 'key': req.headers.get('idempotency-key')}) if '/v1/builder/' in req.url else None)
    page.goto('http://127.0.0.1:3311')
    page.get_by_role('button', name='驗證錢包並開始設計').click()
    composer = page.get_by_label('描述目標或這次想調整的地方')
    expect(composer).to_be_enabled()
    page.get_by_role('button', name='＋ 新增草稿').click()
    expect(page.get_by_text('已保存 · v1', exact=True)).to_be_visible()
    composer.fill('建立完整 WETH / USDC CLMM，零費率，公開成交與庫存上限，名稱 ' + public_title + '。')
    page.get_by_role('button', name='送出 ↗').click()
    expect(page.get_by_role('button', name='停止生成')).to_have_count(0, timeout=20000)
    expect(page.get_by_text('已保存 · v2', exact=True)).to_be_visible()
    provider_conversation = page.get_by_label('選擇策略草稿').input_value()
    page.get_by_role('button', name='私密政策與模板發布').click()
    dialog = page.get_by_role('dialog', name='發布 Provider 模板')
    expect(dialog).to_be_visible()
    expect(dialog.get_by_role('region', name='私密政策編輯器')).to_be_visible()
    # Rule 1 pauses when volatility is high; rule 2 buys in a private range; fallback sells.
    dialog.get_by_label('規則 1 波動下限').fill('200')
    dialog.get_by_role('button', name='＋ 新增規則').click()
    dialog.get_by_label('規則 2 價格下限').fill('2345.12345678')
    dialog.get_by_label('規則 2 價格上限').fill('2678.12345678')
    dialog.get_by_label('規則 2 波動上限').fill('199')
    dialog.get_by_label('規則 2 允許 Maker 買入 WETH').check()
    dialog.get_by_label('規則 2 WETH 單筆上限').fill('0.01')
    dialog.get_by_label('規則 2 USDC 單筆上限').fill('100')
    dialog.get_by_role('button', name='＋ 新增規則').click()
    dialog.get_by_label('規則 3 允許 Maker 賣出 WETH').check()
    dialog.get_by_label('規則 3 WETH 單筆上限').fill('0.02')
    dialog.get_by_label('規則 3 USDC 單筆上限').fill('50')
    # Reordering an unconditional fallback above later rules is rejected locally, before prepare HTTP.
    dialog.get_by_role('button', name='規則 3 上移').click()
    dialog.get_by_role('button', name='規則 2 上移').click()
    dialog.get_by_label('我已確認公開設定、可調範圍及上方私密規則').check()
    before = len([r for r in sent if r['url'].endswith('/templates/prepare')])
    dialog.get_by_role('button', name='加密並審閱版本').click()
    expect(dialog.get_by_role('alert')).to_contain_text('無條件規則會遮住後續規則')
    assert len([r for r in sent if r['url'].endswith('/templates/prepare')]) == before
    dialog.get_by_role('button', name='規則 1 下移').click()
    dialog.get_by_role('button', name='規則 2 下移').click()
    dialog.get_by_label('我已確認公開設定、可調範圍及上方私密規則').check()
    page.set_viewport_size({'width': 390, 'height': 844})
    assert dialog.evaluate('(el) => el.scrollWidth <= el.clientWidth'), 'Private editor horizontal overflow'
    page.screenshot(path=str(artifacts / 'template-private-editor-mobile.png'), full_page=True)
    dialog.get_by_role('button', name='加密並審閱版本').click()
    expect(dialog.get_by_role('button', name='錢包簽名並發布')).to_be_enabled()
    expect(dialog.get_by_label('規則 2 價格下限')).to_be_disabled()
    assert all('2345.12345678' not in r['body'] and '2678.12345678' not in r['body'] for r in sent)

    def lost_commit(route):
        response = route.fetch()
        assert response.status == 200
        route.abort('connectionfailed')
    page.route('**/templates/publish', lost_commit, times=1)
    dialog.get_by_role('button', name='錢包簽名並發布').click()
    expect(dialog.get_by_role('alert')).to_be_visible()
    dialog.get_by_role('button', name='重試確認發布').click()
    expect(dialog.get_by_text('模板已保存，Maker 現在可以選定此版本建立自己的草稿。私密表單已清除。')).to_be_visible()
    expect(dialog.get_by_role('region', name='私密政策編輯器')).to_have_count(0)
    signed = page.evaluate('window.__fixtureSignedMessages')
    assert len([m for m in signed if m.startswith('Publish Pintool Provider template')]) == 1
    requests = [r for r in sent if r['url'].endswith('/templates/publish')]
    assert len(requests) == 2 and requests[0]['body'] == requests[1]['body'] and requests[0]['key'] == requests[1]['key']
    check = page.request.get('http://127.0.0.1:3311/fixture/policy-check')
    assert check.status == 200 and check.json() == {'verified': True, 'scope': 'local-fixture-decryption-and-existing-evaluator', 'teeVerified': False}
    page.screenshot(path=str(artifacts / 'template-published-mobile.png'), full_page=True)
    dialog.get_by_role('button', name='回到策略').click()
    page.get_by_role('button', name='私密政策與模板發布').click()
    expect(dialog.get_by_label('規則 1 價格下限')).to_have_value('')
    expect(dialog.get_by_label('規則 2 價格下限')).to_have_count(0)
    dialog.get_by_role('button', name='關閉視窗').click()
    page.set_viewport_size({'width': 1440, 'height': 1100})
    page.get_by_role('button', name='選擇模板套用').click()
    catalog = page.get_by_role('dialog', name='選擇 Provider 模板')
    catalog.get_by_role('button', name=re.compile(re.escape(public_title) + '.*版本 1')).click()
    expect(catalog.get_by_text('Provider 簽名已核對。版本綁定後，不會自動更新成其他版本。')).to_be_visible()
    catalog.get_by_label('WETH Maker 配置').fill('0.01')
    catalog.get_by_label('USDC Maker 配置').fill('25')
    catalog.get_by_label('個人策略名稱').fill('我的 Maker 策略')
    page.screenshot(path=str(artifacts / 'template-maker-review-desktop.png'), full_page=True)
    page.route('**/templates/instantiate', lost_commit, times=1)
    catalog.get_by_role('button', name='套用並繼續設計').click()
    expect(catalog.get_by_role('alert')).to_be_visible()
    expect(catalog.get_by_label('USDC Maker 配置')).to_be_disabled()
    catalog.get_by_role('button', name='重試建立同一份草稿').click()
    expect(catalog).to_have_count(0)
    expect(page.get_by_text('Maker 草稿 · 未註冊')).to_be_visible()
    expect(page.get_by_text('0.01 WETH ＋ 25 USDC', exact=True)).to_be_visible()
    requests = [r for r in sent if r['url'].endswith('/templates/instantiate')]
    assert len(requests) == 2 and requests[0]['key'] == requests[1]['key'] and requests[0]['body'] == requests[1]['body']
    composer.fill('只把價格上限改成 2700，其他限制保留。')
    page.get_by_role('button', name='送出 ↗').click()
    expect(page.get_by_role('button', name='停止生成')).to_have_count(0, timeout=20000)
    expect(page.get_by_text('已保存 · v2', exact=True)).to_be_visible()
    expect(page.get_by_role('alert')).to_have_count(0)
    summary = page.get_by_role('complementary', name='目前策略')
    expect(summary.get_by_text('2700', exact=True)).to_be_visible()
    page.get_by_role('button', name='選擇模板套用').click()
    catalog.get_by_role('button', name=re.compile(re.escape(public_title) + '.*版本 1')).click()
    catalog.get_by_label('我了解這只會撤下此版本，既有成交授權需另行撤銷').check()
    catalog.get_by_role('button', name='錢包簽名並撤下版本').click()
    expect(catalog.get_by_text(re.compile('Provider 已撤下這個版本'))).to_be_visible()
    expect(catalog.get_by_role('button', name='套用並繼續設計')).to_have_count(0)
    catalog.get_by_role('button', name='關閉視窗').click()
    page.get_by_label('選擇策略草稿').select_option(provider_conversation)
    expect(page.get_by_role('button', name='私密政策與模板發布')).to_be_enabled()
    # An expired prepare response must release its idempotency key so a fresh review can proceed.
    page.get_by_role('button', name='私密政策與模板發布').click()
    def expired_intent(route):
        response = route.fetch()
        assert response.status == 200
        body = response.json()
        body['intent']['expiresAt'] = '2000-01-01T00:00:00.000Z'
        route.fulfill(response=response, json=body)
    page.route('**/templates/prepare', expired_intent, times=1)
    dialog.get_by_label('我已確認公開設定、可調範圍及上方私密規則').check()
    dialog.get_by_role('button', name='加密並審閱版本').click()
    expect(dialog.get_by_role('alert')).to_contain_text('發布審閱已過期')
    dialog.get_by_label('我已確認公開設定、可調範圍及上方私密規則').check()
    dialog.get_by_role('button', name='加密並審閱版本').click()
    expect(dialog.get_by_role('button', name='錢包簽名並發布')).to_be_enabled()
    prepare_requests = [r for r in sent if r['url'].endswith('/templates/prepare')]
    assert prepare_requests[-1]['key'] != prepare_requests[-2]['key']
    dialog.get_by_role('button', name='關閉視窗').click()
    # Expiry during private editing returns to wallet proof and destroys the editor state.
    page.get_by_role('button', name='私密政策與模板發布').click()
    dialog.get_by_label('規則 1 價格下限').fill('2555.87654321')
    page.route('**/templates/prepare', lambda route: route.fulfill(status=401, json={'error': 'authentication-required'}), times=1)
    dialog.get_by_label('我已確認公開設定、可調範圍及上方私密規則').check()
    dialog.get_by_role('button', name='加密並審閱版本').click()
    expect(dialog).to_have_count(0)
    page.get_by_role('button', name='驗證錢包並開始設計').click()
    expect(page.get_by_label('選擇策略草稿')).to_be_enabled()
    page.get_by_label('選擇策略草稿').select_option(provider_conversation)
    page.get_by_role('button', name='私密政策與模板發布').click()
    expect(dialog.get_by_label('規則 1 價格下限')).to_have_value('')
    dialog.get_by_role('button', name='關閉視窗').click()
    assert all('2345.12345678' not in r['body'] and '2678.12345678' not in r['body'] for r in sent)
    assert all('2555.87654321' not in r['body'] for r in sent)
    stored = page.evaluate('JSON.stringify({local: {...localStorage}, session: {...sessionStorage}})')
    assert '2345.12345678' not in stored and '2678.12345678' not in stored
    assert not errors, errors
    print('PASS: ordered private rules, local validation, encryption/evaluator compatibility, no plaintext API/storage, signed publication and retry, clear-on-close, Maker instance and retry, pinned conversation, signed withdrawal, intent/session expiry, mobile; no page errors')
    browser.close()
