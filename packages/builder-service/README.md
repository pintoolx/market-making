...
Node 24+, PostgreSQL 16+ (Railway and CI target 18.6). This package exports an authenticated HTTP handler, durable repositories, an OpenAI/Vercel AI SDK design worker and an event-driven standing-report worker. The existing mandate orchestrator mounts it when `DATABASE_URL` is configured. No public-chain write or CRE deploy is performed by this package.
...
Migration 016 retains orphaned evaluations and permits a new occurrence for re-included logs without overwriting old report references. It cancels only unsent orphan-derived reports. A pre-016 application requires event ingress/workers to stay disabled against the newer schema because its insert uses the replaced uniqueness constraint; retain history and deploy a compatible worker before reenabling. The [gateway and rollback contract](../../docs/BUILDER-PLATFORM-WIRING.md) documents delivery identities, uncertain outcomes and terminal reconciliation.
...
The generic preparation queue uses `FOR UPDATE SKIP LOCKED`, expiring leases and lease tokens. A worker that lost its lease cannot renew or complete the job. Queue payloads/results contain public references and error codes, not arbitrary private inputs. Event delivery uses a separate revision/generation-bound inbox, evaluation lease, per-Maker/strategy nonce sequence, change-only delivery record and resident worker. Active evaluation is serialized per subscription; stop/switch cancels unsent work, delivery rechecks the current binding, and pending reports wait behind earlier unresolved nonces. On restart, rows left in `broadcast` are reconciled through the adapter before any pending report can be sent; they are never blindly rebroadcast. It never signs Maker asset transactions; a delivery adapter must verify the Guard receipt/readback before marking a report accepted.
...
| POST `/drafts/:id/event-subscription` | `{ expectedRevision, artifactId, consentId, source? }`; enables a revision-bound resident event subscription. `source` filters one public adapter; omitted means `*` (all sources) |
...
Twelve tools are wired: capabilities, token resolution, owned Maker inventory reads, patch/restore, validation, compilation, numerical scenario previews, background lifecycle simulation, inspection, non-executable public draft export, and unsigned registration/cancellation plans. Enabling simulation requires a configured worker and `runDesignTurn(..., signal, { simulationEnabled: true })`; otherwise the tool returns `simulation-unavailable`. Inspection can read historical/current simulation summaries without enqueueing. Amount edits use human token units and convert exactly using pinned token decimals. A draft can save one Guard limit or one Maker allocation at a time; validation identifies each missing field and the compiler still requires complete limits and allocations. Untouched settings persist; template tools reject Maker allocation. Pair changes cannot silently relabel existing amounts or price intent. Agent-created plans remain unsigned. The separate Maker wallet UI now performs fresh preflight and explicit signing; no signing tool is exposed to the model. See [wallet execution](../../docs/BUILDER-WALLET-EXECUTION.md).

Compilation uses the `aqua-executor/builder` package entry, which imports the pure Guard compiler, not wallet configuration or contract-artifact loaders. Maker compiles are serialized with the draft revision and any active agent lease; identical revision/profile requests return the same immutable artifact. A durable request receipt retains its original revision after an edit. Read the artifact's current flag before using it; restoring an old specification creates a new revision and does not make its old artifact current. Current compilation does not authorize registration: simulation, user review and current wallet/chain checks remain separate gates. Provider templates are persisted as immutable signed versions and Maker instances retain their pinned version and original permissions.
...
Maker edits/restores and agent tools enforce immutable stored Provider permissions. Original caps/range/deadline are the boundary, with personal title/allocations editable and Pegged adjustments requiring explicit ranges. Model inspection sees public permissions/baseline, never ciphertext. A newer Provider version cannot change an existing pin. Withdrawal prevents new instances but does not revoke an existing standing report; event delivery must perform and confirm that separate action. Migrations 011–016 add immutable public event inbox/cursors, source-filtered generation-bound subscriptions, exact uint64 report sequences, evaluation-bound delivery identities, mutable leases/delivery receipts and a lease-based application outbox. Supply evaluator/delivery adapters to `runEventWorker`; the default service deliberately fails closed without them. For a signed gateway, use `createSignedEventIngress(secret)` and mount it as `eventIngress`; `normalizeMarketUpdate` and `normalizeEvmLog` provide strict source adapters with reorg-safe log identity. `createEvmLogSource` polls confirmed ranges through viem, chunks backfill, persists a bounded block-hash history and returns old identities for reorg cancellation before replay. `createHttpEventGateway` provides a bounded HTTPS boundary for the deployment-owned TEE/CRE evaluator and Guard broadcaster: it sends public identity and events only, validates changed reports and receipts, and redacts upstream errors. The production mandate service now mounts the handler and durable workers when `DATABASE_URL` is configured; set the three event-gateway variables together before enabling report delivery. The orchestrator can mount the EVM source with `BUILDER_EVENT_EVM_ADDRESS` and `BUILDER_EVENT_EVM_FROM_BLOCK`.

## Wallet execution and outage policy (PR #102)

Migration 018 adds the public wallet transaction journal and control plans;
migration 019 adds durable subscription health transitions. See
[wallet execution](../../docs/BUILDER-WALLET-EXECUTION.md) and
[CRE delivery](../../docs/BUILDER-CRE-DELIVERY.md) for runtime options and evidence.

| Route | Purpose |
|---|---|
| GET `/drafts/:id/wallet-transactions` | Owned plan/history list and runtime availability |
| POST `/wallet-transactions/prepare` | `{ planId, step }`; preflight and durable unsigned request |
| POST `/wallet-transactions/:id/reconcile` | Optional public transaction hash; exact receipt/nonce verification |
| POST `/wallet-transactions/:id/rejected` | Close an explicitly unsigned request only when its nonce is unused |
| POST `/drafts/:id/guard-revoke-plan` | Prepare direct Guard revocation |
| POST `/drafts/:id/guard-unrevoke-plan` | Prepare clearing revocation; new report remains required |
| POST `/drafts/:id/allowance-revoke-plan` | Prepare zero shared Aqua allowances |

All mutation routes require wallet ownership and an idempotency key. The browser
cannot supply arbitrary destination/calldata or mark a transaction confirmed.
Registration requires a complete actual fork result; historical recovery does
not require a currently supported recipe. Nonzero fees are now blocked because
of the [reproduced Guard cap issue](../../docs/BUILDER-FEE-GUARD-LIMITATION.md).
The approved checklist and public-chain acceptance limits remain in
[the unmerged PR scope](../../docs/BUILDER-REMAINING-PR.md).
