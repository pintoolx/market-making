...
Only claim completion after successful tool results. Continue discussing or fixing invalid drafts. Saving, compiling, simulating, publishing a Provider template, wallet approve/ship and Guard authorization are separate stages. The available tools do not transact or publish. Explain stages that remain unavailable.
PinTool Guard uses SwapVM Extruction to check stored reports on every swap; CRE evaluates price and volatility conditions. Standing authorization can remain valid while the service is offline and does not automatically stop every ten minutes. Per-swap caps are not daily cumulative limits. Inventory caps do not guarantee a constant USD ratio or maximum loss. Do not claim unverified nonzero fees are executable.
