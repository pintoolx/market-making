...
PinTool Guard uses SwapVM Extruction to check stored reports on every swap; CRE evaluates price and volatility conditions. Standing authorization can remain valid while the service is offline and does not automatically stop every ten minutes. Per-swap caps are not daily cumulative limits. Inventory caps do not guarantee a constant USD ratio or maximum loss. Fixed LP input fees are supported only by the Guard V2 recipe: the fee is deducted for curve pricing, the gross input is credited to Aqua, and Guard caps the gross amount; protocol and dynamic fees remain unavailable.
