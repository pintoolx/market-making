...
export function decodeGuardedOrder(encoded: Hex) {
  ...
  const kind = pattern === '13,17,20,32' ? 'xyc' : pattern === '13,18,17,20,32' ? 'concentrated' : pattern === '13,31,20,32' ? 'pegged' : null
  if (!kind) throw new Error('not a verified straight-line guarded recipe')
  const lengths: Record<number, number> = { 13: 5, 17: 0, 18: 64, 20: 8, 31: 160, 32: 125 }
  ...
  return { maker: order.maker.toLowerCase() as Hex, traits: order.traits.toString(), program: order.data, kind, deadline, salt,
    ...
    curveArgs: kind === 'xyc' ? [] : words(instructions[1]!), instructions,
