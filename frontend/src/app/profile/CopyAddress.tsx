...
// The address itself is the copy button; the icon turns into a check for a moment after copying.
export default function CopyAddress({ address, short = false }: { address: string; short?: boolean }) {
  ...
  return (
    <button type="button" className={aqua.copyAddress} onClick={copy} title={copied ? 'Copied' : 'Copy address'} aria-label={copied ? 'Address copied' : `Copy address ${address}`}>
