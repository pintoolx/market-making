...
type PendingCheck = { hash: Hex; data: Hex; taker: Hex; mandateId?: string; listingId: string };
...
export default function GuardCheck({ onPendingChange, available, choice, wallet, amount, direction }: Props) {
  ...
  const confirm = async (item: PendingCheck) => {
    ...
    if (tx.from.toLowerCase() !== item.taker.toLowerCase() || tx.to?.toLowerCase() !== router.toLowerCase() || tx.input !== item.data || tx.value !== BigInt(0)) throw new Error('The receipt does not match the reviewed request.');
  ...
  const submit = () => act(async () => {
    ...
    if (balance < amountIn || allowance < amountIn) throw new Error('The trading wallet needs sufficient balance and allowance for this request.');
    ...
    const hash = await signer.writeContract({ address: router, abi: tradeAbi, functionName: 'swap', args, gas: BigInt(400000) });
    const mandateId = new URLSearchParams(window.location.search).get('mandate');
    const item: PendingCheck = { hash, data: encodeFunctionData({ abi: tradeAbi, functionName: 'swap', args }), taker, listingId: choice.id,
  ...
  return <details open={!!pending || !!completed}>
    ...
    {!pending && !completed && <>
      ...
      <Secondary disabled={busy || !minimum} onClick={() => void submit()}>Submit verification in wallet</Secondary>
    </>}
    {pending && <><p role="status">Verification transaction submitted.</p><a href={`https://sepolia.etherscan.io/tx/${pending.hash}`} target="_blank" rel="noreferrer">View verification transaction ↗</a><Secondary disabled={busy} onClick={() => void act(() => confirm(pending))}>Check verification receipt</Secondary></>}
