...
const fields: Record<string, string> = { 'spec.baseToken': 'Base token', 'spec.quoteToken': 'Quote token', 'spec.model': 'Market-making curve',
  ...
  'spec.deadline': 'Strategy deadline', 'spec.guardEnvelope': 'Public swap limits', 'spec.title': 'Strategy name', 'requirements': 'Strategy requirements',
  ...
  'maker': 'Liquidity wallet', 'allocations': 'Liquidity amounts', 'allocations.baseAtomic': 'Base-token amount', 'allocations.quoteAtomic': 'Quote-token amount' };
...
export default function BuilderWorkspace({ identity }: { identity: Identity }) {
  ...
  return <section className={styles.workspace} aria-label="Strategy design workspace">
    <header className={styles.heading}><div><span className={styles.eyebrow}>{draft?.kind === 'maker' ? 'Liquidity strategy builder' : 'Strategy builder'}</span><h1>Build from a plain-language brief.</h1><p>Describe the market, pricing model and limits. PinTool turns them into a structured strategy you can inspect and revise.</p></div><span className={styles.network}>Ethereum Sepolia</span></header>
    ...
    {!connected ? <div className={styles.welcome}>
      <div><span className={styles.eyebrow}>Before you publish</span><h2>Compare models and verify every parameter.</h2><p>Each accepted change creates a revision, so you can inspect or restore earlier choices.</p>
      ...
      </div><div className={styles.example}><span>Example brief</span><blockquote>“I want to provide WETH / USDC liquidity and stop trading outside my price range. Help me compare the options.”</blockquote></div>
    </div> : <>
      ...
      <div className={styles.columns}>
        <section className={styles.chat} aria-label="Strategy conversation"><div className={styles.sectionHead}><h2>Strategy assistant</h2><span>Edits public strategy parameters</span></div>
          <div ref={messageArea} className={styles.messages} onScroll={event => {
            ...
            {!messages.length && !busy && <div className={styles.emptyChat}><h3>Describe the market you want to make.</h3><p>Include the token pair, preferred pricing model and any hard limits.</p>{['Compare fixed-range and constant-product curves', 'Help me set per-swap and inventory limits', 'Which conditions can PinTool enforce?'].map(text => <button key={text} onClick={() => setInput(text)}>{text} ↗</button>)}</div>}
          ...
          <form className={styles.composer} onSubmit={submit}><label htmlFor="strategy-message">Describe your goals or what you want to change</label><textarea id="strategy-message" value={input} onChange={e => setInput(e.target.value)} maxLength={8000} rows={3} placeholder="For example: change only the maximum price to 2700 and keep the other limits." disabled={!draft || !!busy} />
            <div className={styles.composerBottom}><span role="status" aria-live="polite">{turnId ? activity || 'Responding' : 'Confidential rules are added before publication.'}</span>{turnId ? <button type="button" onClick={() => void stop()}>Stop generating</button> : <button className={styles.primary} type="submit" disabled={!draft || !input.trim() || !!busy}>Send ↗</button>}</div>
          </form>
        </section>
        <aside className={styles.strategy} aria-label="Current strategy"><div className={styles.sectionHead}><h2>Current strategy</h2><span className={styles.draftTag}>{draft?.kind === 'maker' ? 'Liquidity draft · Not active' : 'Strategy draft'}</span></div>
          {draft ? <>
            ...
            {draft.kind === 'maker' && <div className={styles.makerInstance}><h3>Your liquidity</h3><p>{amount(draft.allocations?.baseAtomic, base?.decimals)} {base?.symbol} + {amount(draft.allocations?.quoteAtomic, quote?.decimals)} {quote?.symbol}</p>
              {templateContext && <><p>Pinned to provider version {templateContext.templatePin.version}. You can change only the parameters the provider allows; later provider updates will not change this draft.</p>
                <details><summary>View editable parameters</summary><ul><li>Personal title and liquidity amounts are editable</li><li>{templateContext.permissions.tightenCaps ? 'The four public swap limits may be tightened' : 'The four public swap limits are fixed'}</li>
                ...
                {templateContext.withdrawn && <p className={styles.notice}>The provider withdrew this version. New liquidity cannot use it; existing onchain authorizations are not automatically revoked.</p>}</>}
              <small>These amounts have not been checked against your wallet and do not enable trading.</small></div>}
            ...
            <div className={styles.guard}><h3>Public swap limits</h3><p>Enforced on every swap. They do not track daily totals.</p><table><thead><tr><th scope="col">Token</th><th scope="col">Maximum per swap</th><th scope="col">Post-swap inventory</th></tr></thead><tbody>
            ...
            {validation?.revision === draft.revision && <div className={styles.validation} data-ready={validation.ready}><strong>{validation.ready ? 'Public parameters complete' : 'More details needed'}</strong>{validation.ready ? <p>{draft.kind === 'maker' ? 'Compile this revision, then test its swap directions and limits before adding liquidity.' : 'Add the confidential trading rules, then review and sign this version for publication.'}</p> : <ul>{validation.missingFields.map(f => <li key={f}>{fields[f] ?? 'Other required parameters'}</li>)}{validation.errors.map((e, i) => <li key={i}>{e.message}</li>)}</ul>}</div>}
            {draft.kind === 'template' && <div className={styles.publishAction}><button className={styles.primary} disabled={!!busy || !!turnId || !validation?.ready || validation.revision !== draft.revision || (draft.requirements.length > 0 && requirementReceipt?.revision !== draft.revision)} onClick={() => setPublisherOpen(true)}>Add private rules and publish</button>
              <p>Set the confidential conditions, review the complete version and sign it in your wallet.</p></div>}
            {draft.kind === 'maker' && <div className={styles.publishAction}><button className={styles.primary} disabled={!!busy || !!turnId || (draft.requirements.length > 0 && requirementReceipt?.revision !== draft.revision)} onClick={() => setPreparationOpen(true)}>Check liquidity and test execution</button>
              <p>Verify wallet capacity and test this revision before enabling it.</p></div>}
