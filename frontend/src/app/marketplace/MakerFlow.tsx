...
export default function MakerFlow({ scrollTop }: { scrollTop: () => void }) {
  ...
  return <section className={aqua.flow}>
    ...
    {phase === 'active' && <div className={aqua.executionLayout}>
      ...
      <div className={aqua.swapTests}><article><span className={aqua.eyebrow}>Permitted flow</span><h3>Sell USDT for USDC</h3><p>Reduces the Maker&apos;s USDT inventory.</p><Primary disabled={swapState === 'settling' || swapState === 'checking'} onClick={() => runSwap(false)}>{swapState === 'settling' ? 'Settling…' : swapState === 'settled' ? 'Settled through Aqua' : 'Execute swap'}</Primary></article><article><span className={aqua.eyebrow}>Protected flow</span><h3>Sell USDC for USDT</h3><p>Guard checks the resulting USDT inventory before settlement.</p><Secondary disabled={swapState === 'settling' || swapState === 'checking'} onClick={() => runSwap(true)}>{swapState === 'checking' ? 'Checking policy…' : swapState === 'blocked' ? 'Blocked by PinTool Guard' : 'Execute swap'}</Secondary></article></div>
