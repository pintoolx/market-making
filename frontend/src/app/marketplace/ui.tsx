...
export function ListingCard({ listing, action }: { listing: Listing; action?: React.ReactNode }) {
  return <article className={`${styles.card} ${aqua.card}`}>
    ...
    <div className={`${styles.cardBody} ${action ? '' : aqua.cardBodyEnd}`}>
      ...
      {listing.provider && <p className={aqua.byline}><Avatar name={listing.provider} src={listing.providerAvatar} size={22} />by <span>{listing.provider}</span></p>}
      ...
      <div className={aqua.cardRule}><span className={aqua.eyebrow}>Kept private by the Provider</span><p>{listing.template.privateInputs}</p></div>
    </div>
