...
export async function readGuardReport({ rpcUrl, guard, maker, strategyHash }, fetchImpl = fetch) {
  const data = encodeFunctionData({ abi: guardAbi, functionName: 'getReport', args: [maker, strategyHash] });
  const result = await rpc(rpcUrl, 'eth_call', [{ to: guard, data }, 'latest'], fetchImpl);
...
export async function findAcceptedReport({ rpcUrl, guard, maker, strategyHash, fromBlock }, fetchImpl = fetch) {
  ...
  const log = logs.at(-1);
  if (!/^0x[0-9a-f]{64}$/i.test(log?.transactionHash ?? '')) throw new Error('RPC returned an invalid report log');
  return { transactionHash: log.transactionHash, blockNumber: BigInt(log.blockNumber) };
}
...
export async function waitForAcceptedReport(config, options = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    if (accepted) {
      ...
      if (stored.report.nonce === 0n || stored.report.strategyHash.toLowerCase() !== config.strategyHash.toLowerCase()
