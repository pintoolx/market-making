...
PinTool uses Chainlink Confidential Workflows to derive a public execution authorization from a private Strategy Provider policy and private Maker limits. The DON signs that authorization, the configured Chainlink forwarder delivers it, and `AquaGuardV2` enforces it during every Aqua quote and swap.
...
The checked-in [Ethereum Sepolia deployment](../contracts/aqua-executor/deployments/11155111.json) uses canonical WETH, Circle testnet USDC and the CRE simulation forwarder. The current receiver is `0x64f6e18e85dae5ec2ee44fe7f9fc66cf2a1f056f`, revision `standing-v2`. It supports schema-2 standing authorization, Maker revocation and receipt recovery.

The [standing authorization verification](../workflow/verification/standing-authorizations/README.md) records this receiver's actual cloud API and Sepolia execution. The [earlier acquisition run](../workflow/verification/live-market-guard/README.md) and [replacement proof](../contracts/aqua-executor/docs/ethereum-sepolia-guard-revision.json) concern previous receivers; their transaction hashes must not be presented as this receiver's evidence. Production DON/TEE execution requires its own assigned workflow identity and is not proven by CLI simulation.
