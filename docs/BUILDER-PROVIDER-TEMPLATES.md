...
2. The separate [browser editor](BUILDER-TEMPLATE-WORKSPACE.md) encrypts its policy for the intent's public key using the existing x25519 / HKDF-SHA256 / XChaCha20-Poly1305 format. Provider AAD is `pintool/confidential-envelope/v1|provider=<lowercase-address>`. The plaintext policy's strategy ID must be `<templateId>.v<version>`. This API never accepts plaintext rules.
