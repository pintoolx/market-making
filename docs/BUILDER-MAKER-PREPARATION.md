...
Requirement acceptance, Maker private limits and trusted bindings remain separate gates. The preparation screen accepts only public Guard report evidence (digest, accepted transaction hash and nonce); the service verifies the exact report against Sepolia RPC before it creates a Maker binding intent. The Maker signs that intent in the wallet, while no asset transaction is signed or sent. A valid binding is required before the separate immutable event-consent intent can be signed and event management enabled. The screen also exposes server-created approve/ship/dock calldata as integrity-checked, unsigned plans with explicit fresh-preflight preconditions. All `registrationReady` values remain false. No executable export or asset transaction is enabled by this screen.
