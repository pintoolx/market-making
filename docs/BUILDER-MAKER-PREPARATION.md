...
The Maker conversation now opens a preparation dialog for its exact draft revision. Users can read current wallet/Aqua inventory, compile and inspect the guarded program, start or cancel a durable background simulation, inspect case outcomes and return to the conversation to make changes. The same dialog now continues into signed event consent and explicit Maker wallet execution. [Wallet execution](BUILDER-WALLET-EXECUTION.md) documents the durable transaction journal, preflight, signing and historical recovery controls added in PR #102.
...
Requirement acceptance, private limits and report authorization remain distinct.
A signed Provider template instance can consent to its first event evaluation
without an existing report. Manually provisioned drafts retain the public Guard
receipt/binding verification flow. Optional source-outage consent names exactly
which observations may request a pause. [CRE delivery](BUILDER-CRE-DELIVERY.md)
describes these branches and their acceptance evidence.

Integrity-checked unsigned plans now feed the explicit wallet controls below
simulations. Each transaction is separately reviewed, preflighted and signed;
compilation, report acceptance, registration and trading retain distinct states.
Historical versions remain selectable for dock/revoke even when their fee
composition is now blocked. Generic `registrationReady` fields remain false;
actual transaction outcomes come from the verified wallet journal.

## Verification

Five frontend unit tests cover public publication/signature verification and Maker preparation consistency checks, including substituted revisions, identities, allocations, program hashes, caps, spender, evidence modes, report evidence fields and unsigned plan integrity. Frontend typecheck, scoped lint and the production static Next build pass.

The browser fixture uses the real React components, wallet session/HTTP/store/agent layers, deterministic compiler, PostgreSQL simulation leases and `runSimulation` lifecycle. Only identity/model, inventory data and settlement adapter are synthetic. The new acceptance verifies insufficient allowance, explicit evidence labels, lost compile/start responses and same-key retries, job cancellation, skipped coverage, reload while a job continues, case detail, failed tiny-allocation simulation, stale results after edits/restores, session expiry and mobile overflow. It asserts that only login and explicit automation-consent/revocation message signatures were requested. Asset signing is covered by the separate wallet fork mode. It does not duplicate or replace the actual-fork engine evidence from the earlier [lifecycle](BUILDER-LIFECYCLE-SIMULATION.md) and [inventory](BUILDER-WALLET-INVENTORY.md) batches.
