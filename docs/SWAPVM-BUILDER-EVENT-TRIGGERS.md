...
Timeouts and malformed gateway responses preserve the `broadcast` state until reconciliation establishes an outcome. Deliver and reconcile use the same durable delivery ID; only a failure proven to precede network dispatch may retry that ID. A gateway-confirmed reverted transaction or terminal fence proving no broadcast marks the record failed, after which a new event may allocate a new report. An empty lookup is unresolved, not permission to resend. The [gateway contract](BUILDER-PLATFORM-WIRING.md) specifies these outcomes and the required fencing behavior.

Migration 016 marks orphaned evaluation occurrences without overwriting their results or delivery references. Reorg cancellation now includes pending reports produced by completed evaluations. Re-inclusion creates a new evaluation ID; canonical duplicate events still deduplicate. Evaluation completion and delivery claims lock the inbox row before their job/subscription/delivery rows, sharing a short transaction boundary with reorg updates. A claim that already crossed the broadcast boundary is retained for receipt reconciliation; cancelling an event cannot undo its transaction.
